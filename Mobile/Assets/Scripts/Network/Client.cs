...
public class Client : MonoBehaviour
{
    ...
    public void OnLoginPress()
    {
        ...
        if (!playerController.isImposter)
        {
            ...
            while (playerController.taskIDs.Count < response.numRemainingTask)
            {
                int newTaskID = UnityEngine.Random.Range(0, 21);
                Debug.Log("Adding new task id: " + newTaskID);
                if (!playerController.taskIDs.Contains(newTaskID))
                {
                    playerController.taskIDs.Add(newTaskID);
                }
            }
        }
        ...
        killButtonText.text = (imposter) ? "Kill" : "Do Task"; 
    ...
    private void FixedUpdate()
    {
        ...
        /*if ( ! VotingListControl.reportInProgress)
        {   
